...
class FilmListService:
    ...
    async def get_by_keyword(self, keyword: str, page: Optional[int] = None) -> Optional[Keyword]:
        ...
        if not self.current_page_data:
            ...
            if tokens_hash == self.keyword_cash_data.tokens_hash:
                if not page: page = self.current_page_data.scroll_page + 1
    ...
    async def _film_from_cache(self, keyword: str, page: Optional[int]) -> Optional[Film]:
        ...
        self.current_page_data = PageData(scroll_page=page,
                                          results_ids=cashed_keyword.pages[str(page)],)
    ...
    async def _get_film_from_elastic(self, keyword: str, page_number: int) -> Optional[Keyword]:
        query = {"size": 10,
                 "query": {"bool": {"must": [{"match": {"description": keyword}}]}}}
    ...
    async def _get_keyword_hash(self, keyword: str):
        ...
        token_list.sort()
        tokens_hash = hashlib.sha256(''.join(token_list).encode('utf8')).hexdigest()
    ...
    async def _scroll_film_from_elastic(self):
        ...
        film_list = self.film_list_from_response(responce)
        self.current_page_data.results_ids = [film['_source']['id'] for film in film_list]
